...
export function NotificationPopover({ workspaceId }: NotificationPopoverProps) {
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        align="end"
        className="w-80 sm:w-110 p-0 shadow-2xl border border-border/80 rounded-2xl backdrop-blur-xl bg-popover/95 overflow-hidden transition-all"
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-border/60 p-4 px-4.5 bg-muted/40 gap-3">
          <div className="flex items-center gap-2.5 min-w-0">
            <BellIcon className="size-4.5 text-primary shrink-0" />
            <h3 className="text-sm font-bold text-foreground shrink-0">Thông báo</h3>
            {unreadCount > 0 && (
              <span className="rounded-full bg-primary/15 text-primary px-2.5 py-0.5 text-xs font-semibold shrink-0 border border-primary/20">
          ...
          {unreadCount > 0 && (
            <Button
              ...
              className="h-8 px-2.5 text-xs font-semibold text-muted-foreground hover:text-primary hover:bg-primary/10 gap-1.5 shrink-0 rounded-lg cursor-pointer transition-colors"
            >
          ...
          {isLoading ? (
            ...
            notifications.map((notification) => (
              <div
                ...
                className={cn(
                  'flex items-start gap-3.5 p-4 transition-colors cursor-pointer hover:bg-accent/60',
                  !notification.isRead && 'bg-primary/5 dark:bg-primary/10'
                )}
              >
                <div className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-background border border-border/60 shadow-xs mt-0.5">
                  {getNotificationIcon(notification.type)}
                </div>

                <div className="flex-1 min-w-0 space-y-1">
                  <div className="flex items-center justify-between gap-2">
                    <p className={cn('text-xs font-semibold text-foreground truncate', !notification.isRead && 'font-bold text-primary')}>
                      {notification.title}
                    </p>
